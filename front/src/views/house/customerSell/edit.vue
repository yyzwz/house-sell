<template>
  <div>
    <Card>
      ...
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate" label-position="left" >        <FormItem label="房屋ID" prop="houseId"  >
          <Input v-model="form.houseId" clearable style="width:570px"/>
        </FormItem>
        ...
        <FormItem label="价格" prop="houseMoney"  >
        ...
        <FormItem label="买家ID" prop="userId"  >
...
<script>
import { editCustomerSell } from "./api.js";
export default {
  name: "edit",
  components: {
    },
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          editCustomerSell(this.form).then(res => {
