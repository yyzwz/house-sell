...
<div class="search">
    ...
    <Modal v-model="frontOrderModal" title="意向预定" @on-ok="frontLeaseFx">
        <Row :gutter="16">
            <Form :label-width="100" label-position="left" style="width:90%">
                <FormItem label="价格（元）" prop="houseMoney">
...
export default {
                        ...
                        return h("div", [
                            h(
                                "Button", {
                                    ...
                                    on: {
                                        click: () => {
                                            this.openFrontLeaseFx(params.row);
                                        }
                                    }
                                },
                                "出租预定"
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getHouseLeaseList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
