...
public class DateUtils {
    ...
    public static String getBirthday(String idcard){
        if(idcard == null || NullUtils.isNull(idcard) || idcard.length() < 17)
            return null;
    ...
    public static String starToChinese(int star){
        if(star == 0) return "星期日";
        if(star == 1) return "星期一";
        if(star == 2) return "星期二";
        if(star == 3) return "星期三";
        if(star == 4) return "星期四";
        if(star == 5) return "星期五";
        return "星期六";
    }
